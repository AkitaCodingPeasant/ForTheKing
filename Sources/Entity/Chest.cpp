...
Chest::Chest() { position = { -69, -69 }; }
Chest::Chest(int x, int y) { position = { x,y }; }
Chest::Chest(int x, int y, std::string name, std::vector<std::string> treasures) {
    this->position = { x,y };
    this->name = name;
...
void Chest::GiveTreasureTo(Role* role) {
    ...
    if (treasures.empty()) {
        ...
        std::vector<std::string> randStr2 = { "經過一番搜索","什麼事都沒有發生，被路過的戴文凱施捨一些東西","坐在附近發呆，總之就是...","感到非常興奮","感到自己被幸運之神眷顧","仔細觀察隻遭" };
        ...
        for (const auto& pair : ItemTable::itemMap) {
